...
for entry in tqdm(
    ...
    if (
        ...
        N_med = np.int(
            len(Data) / ((int(silence_duration) + int(sweep_duration)) * fs2)
        )
            ...
            if fs2 == fs:
                ...
                impulso = (
                    signal.convolve(
                        Data[
                            n
                            * ((silence_duration + sweep_duration) * fs2) : (
                                n * ((silence_duration + sweep_duration) * fs2)
                            )
                            + ((silence_duration + sweep_duration) * fs2)
                        ],
                        filtroinverso,
                ...
                sf.write(
                    Directorio + "/RIRs/" + name + "_" + str(n) + "_RIR" + ".wav",
                    impulso,
                    fs,
                    subtype="PCM_16",
