...
for entry in tqdm(os.listdir(Directorio)): #Lista todo lo que este en el directorio. El tqdm es la barra de estado, asi como esta la barra de estado cuenta todos los archivos que haya en la carpeta, no solo los audios, tengo que ver como corregirlo.
	if os.path.isfile(os.path.join(Directorio,entry)) and os.path.splitext(entry)[1]==".wav": #Si el elemento de la lista es un archivo (o sea no una carpeta) y ademas es .wav sigue
		...
		#print(size(Data))
		if fs2 == fs:
			impulso = signal.convolve(Data,filtroinverso,mode = 'same') / sumfil  #Convoluciona el audio con el filtro inverso
			impulso = impulso[np.where(impulso == np.amax(impulso))[0][0] - fs :] # Corta el audio desde un segundo antes del maximo, hay que ver si esta bien esto
			impulso = (impulso/max(impulso)) * 0.8 #normalizo el audio
			sf.write(Directorio + "/" + name + "_RIR" + ".wav",impulso,fs,subtype='PCM_16') #Guarda el impulso en 16 bits
		else:
			if 'filtroinverso2' in locals():
				impulso = signal.convolve(Data,filtroinverso2,mode = 'same') / sumfil2#FFFFFF#FFFFFF  #Convoluciona el audio con el filtro inverso
				...
				impulso = (impulso/max(impulso)) * 0.8 #normalizo el audio
				sf.write(Directorio + "/" + name + "_RIR" + ".wav",impulso,fs2,subtype='PCM_16') #Guarda el impulso en 16 bits
			else:
				...
				impulso = signal.convolve(Data,filtroinverso2,mode = 'same') / sumfil2  #Convoluciona el audio con el filtro inverso
				impulso = impulso[np.where(impulso == np.amax(impulso))[0][0] - fs2 :] # Corta el audio desde un segundo antes del maximo, hay que ver si esta bien esto
				impulso = (impulso/max(impulso)) * 0.8 #normalizo el audio
				sf.write(Directorio + "/" + name + "_RIR" + ".wav",impulso,fs2,subtype='PCM_16') #Guarda el impulso en 16 bits
